...
class Signer {
  static sign(
    ...
    List<String> sortedqueryKeys = url.queryParameters.keys.toList()..sort();
    String canonicalQuery = sortedqueryKeys
        .map((key) => Uri.encodeFull('$key=${url.queryParameters[key]}'))
        .join('&');
    ...
    String payloadHash = headers['X-Amz-Content-Sha256'] ?? sha256.convert(utf8.encode(body ?? '')).toString();
    String canonical = ([
      method.toUpperCase(),
      Uri.encodeFull(url.path),
      canonicalQuery,
    ]
